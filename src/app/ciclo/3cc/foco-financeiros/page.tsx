...
import { receitaFin, receitaFinCoolseg, v3FocoFinanceirosColab, v3ProximoPatamar } from '@/lib/compute3cc';
  ...
  return (
    <div className="space-y-8 p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="bg-white rounded-xl p-4 shadow"><div className="text-xs uppercase tracking-wide text-slate4">Receita Coolseg (Fin)</div><div className="text-2xl font-bold mt-1">{fmtEUR(totalRec)}</div></div>
        <div className="bg-white rounded-xl p-4 shadow"><div className="text-xs uppercase tracking-wide text-slate4">Total prémios V3</div><div className="text-2xl font-bold mt-1">{fmtEUR(totalPremios)}</div></div>
        <div className="bg-white rounded-xl p-4 shadow"><div className="text-xs uppercase tracking-wide text-slate4">Colabs elegíveis</div><div className="text-2xl font-bold mt-1">{s.colaboradores.filter(c => v3FocoFinanceirosColab(s, c.id) > 0).length}<span className="text-sm text-slate4"> / {s.colaboradores.length}</span></div></div>
      </div>
      <section>
        ...
        <div className="bg-white rounded-xl shadow overflow-x-auto">
          <table className="text-sm w-full">
            <thead className="bg-gray-100 text-xs"><tr>
              ...
              <th className="text-right px-3 py-2">Receita Fin (€)</th><th className="text-right px-3 py-2">Prémio actual</th>
                  ...
                  return (
                    <tr key={c.id} className="border-t">
                      ...
                      <td className="px-3 py-1.5 text-xs text-slate4">
                        {prox ? <>faltam <strong>{fmtEUR(prox.falta)}</strong> para <strong>{fmtEUR(prox.proximo)}</strong> (@{fmtEUR(prox.alvo)})</> : <span className="text-green-700">✓ Tecto atingido</span>}
                      </td>
                    </tr>
                  );
                });
