...
import {
  empSaldo, empSaldoCoolseg, objColabValue, minFidEmpRamo,
...
export default async function Maratona3ccPage() {
  ...
  // Ramos que contam para a majoração 50% (SEE é gate, fica fora)
  const ramosMajoracao = ['Multicare', 'PVE', 'Responsabilidade Civil', 'Propriedades Digitais'];
    ...
    <div className="space-y-8 p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="bg-white rounded-xl p-4 shadow">
          <div className="text-xs uppercase tracking-wide text-slate4">Receita Empresas total</div>
          <div className="text-2xl font-bold mt-1">{fmtEUR(totalReceita)}</div>
        </div>
        <div className="bg-white rounded-xl p-4 shadow">
          <div className="text-xs uppercase tracking-wide text-slate4">Total incentivos V2</div>
          <div className="text-2xl font-bold mt-1">{fmtEUR(totalIncentivos)}</div>
        </div>
        <div className="bg-white rounded-xl p-4 shadow">
          <div className="text-xs uppercase tracking-wide text-slate4">Colabs c/ majoração</div>
          <div className="text-2xl font-bold mt-1">
            {s.colaboradores.filter(c => v2CicloCumprido(s, c.id)).length}
          ...
          <table className="text-sm w-full">
            <thead className="bg-gray-100 text-xs">
              <tr>
                ...
                <th className="text-right px-3 py-2">Receita Emp (€)</th>
                <th className="text-right px-3 py-2">Base (750€×30)</th>
                <th className="text-center px-3 py-2">Cumpre ciclo?</th>
                <th className="text-right px-3 py-2">Majoração +50%</th>
                  ...
                  return (
                    <tr key={c.id} className="border-t">
                      ...
                      <td className="px-3 py-1.5 text-center">
                        {cumpre
                          ? <span className="text-[10px] px-2 py-0.5 rounded bg-green-100 text-green-800 font-semibold">SIM</span>
                          : <span className="text-[10px] px-2 py-0.5 rounded bg-slate-100 text-slate4">não</span>}
                      </td>
                      <td className={`px-3 py-1.5 text-right ${bonus > 0 ? 'text-green-700 font-semibold' : 'text-slate4'}`}>
