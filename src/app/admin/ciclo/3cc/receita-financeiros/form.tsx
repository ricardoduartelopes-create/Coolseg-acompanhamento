...
export default function ReceitaForm3cc({
  campo, colaboradores,
}: {
  campo: 'receita_empresas' | 'receita_financeiros';
  colaboradores: Colab[];
}) {
  ...
  async function handleSave() {
    ...
    for (const c of colaboradores) {
      ...
      let num: number;
      if (raw === '') {
        // Se estava vazio E o valor original também era 0/null, saltar (nada a fazer).
        // Se estava vazio MAS a BD tinha valor > 0, enviar 0 para limpar.
        if (c.valor > 0) num = 0;
        else continue;
      } else {
        num = Number(raw.replace(/\s/g, '').replace(',', '.'));
    ...
    <div className="space-y-3">
      <div className="flex items-center gap-2 justify-end">
        <button onClick={handleSave} disabled={status==='saving'}
                className="bg-head text-white px-4 py-1.5 rounded font-semibold disabled:opacity-50 text-sm">
            ...
            {colaboradores.map(c => (
              <tr key={c.id} className="border-t hover:bg-gray-50">
                ...
                <td className="px-3 py-1">
                  <input type="text" value={vals[c.id] ?? ''}
                         onChange={e => setVals(s => ({ ...s, [c.id]: e.target.value }))}
      ...
      <p className="text-xs text-slate4">
        Aceita vírgula ou ponto como separador decimal. Limpa o campo e Guarda para pôr a 0.
      </p>
    </div>
  );
}
