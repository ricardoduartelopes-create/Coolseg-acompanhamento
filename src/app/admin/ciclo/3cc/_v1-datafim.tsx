...
export default function V1DataFimEditor3cc({ initial }: { initial: string }) {
  ...
  async function save(newValue: string) {
    ...
    const res = await fetch('/api/settings', {
      ...
      body: JSON.stringify({ key: 'v1_data_fim_3cc', value: newValue }),
    });
    if (!res.ok) {
      const d = await res.json().catch(() => ({}));
      setStatus('error'); setError(d.error ?? 'erro'); return;
    }
    setValue(newValue); setStatus('saved');
    setTimeout(() => window.location.reload(), 600);
  }
  function setToToday() {
    const d = new Date();
    save(`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`);
  }
  ...
  return (
    <div className={`bg-white rounded-xl shadow p-5 border-2 ${isActive ? 'border-amber-300 bg-amber-50/20' : 'border-slate3'}`}>
      <div className="flex items-baseline justify-between gap-3 flex-wrap mb-2">
        <div className="text-sm uppercase text-slate4">Encerramento da Velocidade · 3CC</div>
      ...
      <p className="text-sm text-slate4 mt-2 mb-3">
        Se definida, a Velocidade só conta apólices com data ≤ à data indicada.
      </p>
      <div className="flex gap-2 flex-wrap items-center">
        <input type="date" value={value} onChange={e => setValue(e.target.value)}
               className="px-3 py-2 border border-slate3 rounded text-sm min-w-[160px]"/>
        <button onClick={() => save(value)} disabled={status==='saving'}
        ...
        <button onClick={setToToday} disabled={status==='saving'}
