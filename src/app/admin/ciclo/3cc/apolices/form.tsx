...
export default function ApoliceForm3cc({
  ...
  const ramosPartUteis = useMemo(
    () => ramosPart.filter(r => r !== 'Financeiros'),
    [ramosPart]
  );

  const ramoMap = useMemo(() => ({
    particulares_novas: ramosPartUteis,
    particulares_anuladas: ramosPartUteis,
    empresas_novas: ramosEmp,
    empresas_anuladas: ramosEmp,
    diversificacao: ramosDiv,
  } as const), [ramosPartUteis, ramosEmp, ramosDiv]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/apolices-3cc', {
      method: 'POST',
